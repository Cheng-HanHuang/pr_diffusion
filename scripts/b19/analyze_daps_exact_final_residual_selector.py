...
def main() -> None:
    ...
    for sample_path in sorted(Path(args.samples_dir).glob("*run*.png")):
        ...
        with torch.no_grad():
            ...
            row = {
                ...
                "exact_relative_measurement_residual": float(rel_residual.detach().cpu().item()),
                "exact_operator_loss": loss,
            }
    ...
    fieldnames = list(rows[0].keys()) if rows else [
        "run_index",
        "sample_path",
        "exact_relative_measurement_residual",
        "exact_operator_loss",
    ]
    ...
    if rows:
        best_res = min(rows, key=lambda r: r["exact_relative_measurement_residual"])
        print("\nSELECT BY MIN EXACT RELATIVE MEASUREMENT RESIDUAL")
        print(best_res)

        finite_loss_rows = [r for r in rows if math.isfinite(float(r["exact_operator_loss"]))]
        if finite_loss_rows:
            best_loss = min(finite_loss_rows, key=lambda r: r["exact_operator_loss"])
