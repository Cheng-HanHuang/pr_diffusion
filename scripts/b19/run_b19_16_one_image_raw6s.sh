...
ID="${1:?usage: run_b19_16_one_image_raw6s.sh IMAGE_ID PHYSICAL_GPU [SEED]}"
PHYS_GPU="${2:?usage: run_b19_16_one_image_raw6s.sh IMAGE_ID PHYSICAL_GPU [SEED]}"
SEED="${3:-3000}"
...
NAME="b19_daps_${ID}_meas${SEED}_6S_rawtraj"
SAVE_DIR="results/b19_16_full25_raw6S_seed${SEED}"
...
if [ ! -f "$METRICS" ]; then
  ...
  CUDA_VISIBLE_DEVICES="$PHYS_GPU" python posterior_sample.py \
    ...
    name="$NAME" \
    gpu=0 2>&1 | tee "$BASE/B19_16_daps6S_${ID}_meas${SEED}_rawtraj.log"
else
...
python scripts/b19/analyze_daps_rawtraj_early_features.py \
  ...
  --out_step_csv "$BASE/B19_16_daps_${ID}_meas${SEED}_6S_rawtraj_step_features.csv" \
  --out_window_csv "$BASE/B19_16_daps_${ID}_meas${SEED}_6S_rawtraj_window_features.csv"
...
python scripts/b19/analyze_daps_exact_final_loss_selector.py \
  ...
  --measurement_path "$MEAS" \
  --metrics_json "$METRICS" \
  --out_csv "$BASE/B19_16_daps6S_${ID}_meas${SEED}_exact_final_loss_selector.csv"
