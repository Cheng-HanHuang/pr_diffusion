...
ID="${1:?usage: run_b20_7a_one_image_raw16_traj.sh IMAGE_ID PHYSICAL_GPU [MEAS_SEED] [RUN_SEED]}"
PHYS_GPU="${2:?usage: run_b20_7a_one_image_raw16_traj.sh IMAGE_ID PHYSICAL_GPU [MEAS_SEED] [RUN_SEED]}"
MEAS_SEED="${3:-5001}"
RUN_SEED="${4:-4400}"
...
NAME="b20_7a_rawtraj_${ID}_meas${MEAS_SEED}_runseed${RUN_SEED}_${NUM_RUNS}S"
SAVE_DIR="results/b20_7a_rawtraj_${NUM_RUNS}S_meas${MEAS_SEED}_runseed${RUN_SEED}"
ROOT="$REPO/external/daps/$SAVE_DIR/$NAME"
METRICS="$ROOT/metrics.json"

EXACT_OUT="$BASE/B20_7A_daps${NUM_RUNS}S_${ID}_meas${MEAS_SEED}_runseed${RUN_SEED}_exact_final_loss_selector.csv"
RUN_LOG="$BASE/B20_7A_daps${NUM_RUNS}S_${ID}_meas${MEAS_SEED}_runseed${RUN_SEED}.log"
EXACT_LOG="$BASE/B20_7A_daps${NUM_RUNS}S_${ID}_meas${MEAS_SEED}_runseed${RUN_SEED}_exact_selector.log"
WINDOW_OUT="$BASE/B20_7A_daps_${ID}_meas${MEAS_SEED}_runseed${RUN_SEED}_${NUM_RUNS}S_rawtraj_window_features.csv"
WINDOW_LOG="$BASE/B20_7A_daps_${ID}_meas${MEAS_SEED}_runseed${RUN_SEED}_${NUM_RUNS}S_rawtraj_window_features.log"
...
if [ ! -f "$METRICS" ]; then
  echo "[run rawtraj] image=$ID meas_seed=$MEAS_SEED run_seed=$RUN_SEED num_runs=$NUM_RUNS gpu=$PHYS_GPU"
  ...
  set -e
  if [ "$status" -ne 0 ]; then
    echo "[error] rawtraj DAPS run failed"
    exit "$status"
  fi
else
...
if [ ! -f "$WINDOW_OUT" ]; then
  python scripts/b19/analyze_daps_rawtraj_early_features.py \
    ...
    --measurement_path "$MEAS" \
    --metrics_json "$METRICS" \
    --out_csv "$WINDOW_OUT" > "$WINDOW_LOG" 2>&1
fi

echo "[done] B20.7A rawtraj image=$ID meas=$MEAS_SEED runseed=$RUN_SEED"
