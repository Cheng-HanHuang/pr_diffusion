...
def main() -> None:
        ...
        for run_seed in RUN_SEEDS:
            for meas_seed in MEAS_SEEDS:
                ...
                for K in K_GRID:
                    ...
                    good = cand[cand[col] >= 25.0].sort_values("run_index")

                    rows.append({
                        "image_id": image_id,
                        "run_seed": run_seed,
                        "meas_seed": meas_seed,
                        "num_runs_available": n_avail,
                        "K": K,
                        "oracleK_psnr": float(oracle[col]),
                        "oracleK_run": int(oracle["run_index"]),
                        "oracle_good25": int(float(oracle[col]) >= 25.0),
                        "oracle_bad25": int(float(oracle[col]) < 25.0),
    ...
    per_path = BASE / "B20_4A_init_oracle_only_per_case.csv"
    ...
    by_K_path = BASE / "B20_4A_init_oracle_only_by_K.csv"
    ...
    by_image_K_path = BASE / "B20_4A_init_oracle_only_by_image_K.csv"
    by_image_K.to_csv(by_image_K_path, index=False)
    print("[write]", by_image_K_path)

    # Largest K per image/run/meas.
    ...
    by_image_seed_path = BASE / "B20_4A_init_oracle_only_by_image_runseed.csv"
    ...
    first_good_path = BASE / "B20_4A_init_oracle_only_first_good_summary.csv"
    ...
    print("\n== B20.4A initialization-only oracle summary by K ==")
    print(by_K.to_string(index=False))

    print("\n== B20.4A initialization-only oracle by image at K=16 ==")
    if 16 in set(by_image_K["K"]):
        print(
            by_image_K[by_image_K["K"] == 16][[
                "image_id", "cases", "run_seeds",
                "mean_oracleK_psnr", "min_oracleK_psnr", "max_oracleK_psnr",
                "oracle_good25", "oracle_bad25", "mean_num_good_runs_in_K",
            ]]
            .to_string(index=False)
        )

    print("\n== B20.4A initialization-only oracle by image/run_seed at largest K ==")
    print(by_image_seed.to_string(index=False))

    print("\n== B20.4A initialization-only first good run summary ==")
    if len(first_good):
