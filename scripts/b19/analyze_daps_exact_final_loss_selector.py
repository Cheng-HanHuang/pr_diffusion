...
def main() -> None:
    ...
    for sample_path in sorted(Path(args.samples_dir).glob("*run*.png")):
        ...
        with torch.no_grad():
            try:
                exact_loss = scalarize_loss(operator.loss(x, y))
            except Exception as e:
                exact_loss = math.nan
                print(f"[warn] loss failed for run {run_i}: {e}")
            ...
            if math.isfinite(exact_loss):
                # This is only a scale-normalized proxy. Ranking should use exact_operator_loss.
                y_norm = float(y.flatten(1).norm(dim=1)[0].detach().cpu().item())
                row["sqrt_loss_over_y_norm"] = math.sqrt(max(exact_loss, 0.0)) / (y_norm + 1e-12)
            else:
                row["sqrt_loss_over_y_norm"] = math.nan
    ...
    if not rows:
        raise RuntimeError(f"No sample PNGs found in {args.samples_dir}")
    ...
    finite = [r for r in rows if math.isfinite(float(r["exact_operator_loss"]))]
    if finite:
        best_loss = min(finite, key=lambda r: r["exact_operator_loss"])
        print("\nSELECT BY MIN EXACT OPERATOR LOSS")
        print(best_loss)
