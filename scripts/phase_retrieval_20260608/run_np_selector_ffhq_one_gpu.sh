...
# Lightweight LF-vs-S2 selector baseline on FFHQ, no refinement.
# This is useful as the direct control for run_np_refine_ffhq_one_gpu.sh.

GPU_ID="${1:-0}"
TAG="${2:-ffhq_full25_s100_103_selector_control}"
...
OUTDIR="$OUT_ROOT/np_selector_ffhq/$TAG"
mkdir -p "$OUTDIR"
...
CUDA_VISIBLE_DEVICES="$GPU_ID" python scripts/pr_external_difffpr_np_guided_lf_s2_selector.py \
  --data_root "$DATA_ROOT" \
