#!/usr/bin/env python3
"""Run one B24.2 cumulative-7424 extension shard (rows 6144--7423 only).

The completed cumulative-6144 campaign is the immutable parent checkpoint. This
worker executes only deterministic new rows assigned to one fixed physical GPU.
It preserves the B24.1-validated four-independent-process scheduling form for
DAPS-4 and SITCOM-4.

The calibrated baseline admission gate defaults to 10240 MiB. Transient lack of
free memory causes a recorded wait rather than shard failure. Completed rows are
atomically resumable. On resume, only exact B24-7424-owned DAPS scratch for an
incomplete row may be removed before that row is regenerated.
"""
...
DAPS_ROOT = Path("/egr/research-pac/huang248/pr_diffusion_b19_solver/external/daps")
PARENT_COUNT = 6144
TARGET_COUNT = 7424
EXPECTED_ROWS_PER_SHARD = (TARGET_COUNT - PARENT_COUNT) // 4  # 320
PARENT_EXTENSION_ROWS_PER_SHARD = 1024
POLL_SECONDS = 60
...
def validate_parent_shard(
    ...
    if not summary_path.is_file():
        raise RuntimeError(f"missing completed 6144 parent shard summary: {summary_path}")
    value = read_json(summary_path)
    if (
        value.get("status") != "PASS"
        or value.get("stage") != "B24.2_6144_EXTENSION"
        or int(value.get("completed", -1)) != PARENT_EXTENSION_ROWS_PER_SHARD
        or value.get("manifest_file_sha256") != parent_manifest_sha
    ):
        raise RuntimeError(f"bad parent-6144 shard summary: {summary_path}: {value}")
    actual = sum(1 for _ in (parent_runroot / f"shard{shard}").rglob("IMAGE_COMPLETE.json"))
    if actual != PARENT_EXTENSION_ROWS_PER_SHARD:
        raise RuntimeError(
            f"parent-6144 shard {shard} completion count mismatch: {actual} != {PARENT_EXTENSION_ROWS_PER_SHARD}"
        )
    print(
        f"PARENT6144_READY|shard={shard}|completed={value['completed']}|summary={summary_path}",
        flush=True,
    )
    return value
...
def run_group_after_fit(smoke, specs, gpu: int, memory_path: Path, label: str):
    # Override the generic B24.1 52-GiB default only for this calibrated baseline stage.
...
def cleanup_daps_scratch(data_name: str) -> None:
    """Remove only the exact B24-7424 scratch pair for an incomplete row."""
    if not SAFE_NAME.fullmatch(data_name) or not data_name.startswith("b24-7424-"):
        raise RuntimeError(f"refusing unsafe/non-B24-7424 scratch cleanup: {data_name!r}")
    dataset_root = (DAPS_ROOT / "dataset").resolve()
    config_root = (DAPS_ROOT / "configs/data").resolve()
    data_dir = DAPS_ROOT / "dataset" / data_name
    config_path = DAPS_ROOT / "configs/data" / f"{data_name}.yaml"
    if data_dir.parent.resolve() != dataset_root or config_path.parent.resolve() != config_root:
        raise RuntimeError(f"unsafe scratch paths for {data_name}")
    removed = []
    ...
    if removed:
        print(
            f"STALE_DAPS_SCRATCH_REMOVED|name={data_name}|paths={'|'.join(removed)}",
            flush=True,
        )
...
def main() -> int:
    ...
    parent_manifest_path = parent_runroot / "B24_2_baseline_6144.json"
    ...
    if rows[:PARENT_COUNT] != parent_rows:
        raise RuntimeError("cumulative-7424 manifest does not preserve exact 6144-row prefix")

    parent_sha = sha256_file(parent_manifest_path)
    validate_parent_shard(parent_runroot, args.shard, parent_sha)
    ...
    smoke = load_module(
        repo / "scripts/b24/run_b24_1_method_smoke.py", "b24_smoke_reuse_7424"
    )
    smoke.MIN_FREE_MIB = CALIBRATED_MIN_FREE_MIB
    ...
    for row in selected:
        ...
        data_name = (
            f"b24-7424-{out.parent.name}-s{args.shard}-r{row_index:04d}-{image_id}"
        ).lower()
        ...
        run_logged(
            [
                str(DAPS_PY),
                str(repo / "scripts/b24/generate_b24_locked_input.py"),
                "--image-id",
                image_id,
                "--measurement-seed",
                str(int(row["measurement_seed"])),
                "--output",
                str(input_dir),
            ],
            cwd=repo,
            env=env,
            log=image_dir / "logs/input_generation.log",
        )
        ...
        try:
            ...
            daps = run_group_after_fit(
                ...
                daps_specs,
                args.gpu,
                image_dir / "daps_memory.tsv",
                f"row{row_index}:DAPS",
            )
            total_wait += max(
                0.0,
                time.perf_counter() - before - float(daps["group_wall_seconds"]),
            )
        finally:
        ...
        for spec in sitcom_specs:
            ...
            mval["selection_rule"] = (
                "B24.2 deterministic cumulative-7424 top-up row; "
                "frozen image/measurement/solver seeds"
            )
            mval["selection_uses_method_outcome"] = False
            mval["b24_stage"] = "B24.2_7424_EXTENSION"
        ...
        sitcom = run_group_after_fit(
            ...
            sitcom_specs,
            args.gpu,
            image_dir / "sitcom_memory.tsv",
            f"row{row_index}:SITCOM",
        )
        total_wait += max(
            0.0,
            time.perf_counter() - before - float(sitcom["group_wall_seconds"]),
        )
        ...
        run_logged(
            [
                str(CTRL_PY),
                str(repo / "scripts/b24/evaluate_b24_baseline_image.py"),
                "--image-id",
                image_id,
                "--ground-truth",
                item["ground_truth_tensor_path"],
                "--daps-group",
                str(image_dir / "DAPS_GROUP.json"),
                "--sitcom-group",
                str(image_dir / "SITCOM_GROUP.json"),
                "--output",
                str(image_dir / "metrics"),
            ],
            cwd=repo,
            env=metric_env,
            log=image_dir / "logs/metrics.log",
        )
        ...
        completion = {
            ...
            "stage": "B24.2_7424_EXTENSION",
            "manifest_file_sha256": manifest_sha,
            "parent_6144_manifest_file_sha256": parent_sha,
