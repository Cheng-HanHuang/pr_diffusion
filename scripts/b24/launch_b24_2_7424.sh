...
PARENT_POINTER="$OUTROOT/B24_2_6144_LATEST_RUN.txt"
...
[[ -f "$PARENT_POINTER" ]] || { echo "STOP|missing_parent6144_pointer:$PARENT_POINTER"; exit 2; }
PARENT6144=$(cat "$PARENT_POINTER")
PARENT_MANIFEST="$PARENT6144/B24_2_baseline_6144.json"
[[ -d "$PARENT6144" ]] || { echo "STOP|missing_parent6144:$PARENT6144"; exit 2; }
[[ -f "$PARENT_MANIFEST" ]] || { echo "STOP|missing_parent6144_manifest:$PARENT_MANIFEST"; exit 2; }
...
"$PY" - "$PARENT6144" "$PARENT_MANIFEST" "$PARENT_SHA" <<'PY'
import json,sys
from pathlib import Path
root=Path(sys.argv[1]); mp=Path(sys.argv[2]); sha=sys.argv[3]
v=json.loads(mp.read_text())
assert len(v['rows']) == 6144
assert [int(r['row_index']) for r in v['rows']] == list(range(6144))
for g in range(4):
    sp=root/f'shard{g}/SHARD_COMPLETE.json'
    assert sp.is_file(), sp
    s=json.loads(sp.read_text())
    assert s.get('stage') == 'B24.2_6144_EXTENSION', (g,s)
    assert s.get('status') == 'PASS', (g,s)
    assert int(s.get('completed',-1)) == 1024, (g,s)
    ...
    assert actual == 1024, (g,actual)
print(f'PARENT6144_READY|path={root}|manifest_sha256={sha}|extension_rows=4096|shards=4x1024')
PY

STAMP=$(date -u +%Y%m%dT%H%M%SZ)
RUNROOT="$OUTROOT/B24_2_7424_extension_$STAMP"
mkdir -p "$RUNROOT/logs" "$RUNROOT/pids"
MANIFEST="$RUNROOT/B24_2_baseline_7424.json"
"$PY" scripts/b24/render_b24_baseline_manifest.py --count 7424 --out "$MANIFEST" \
...
assert len(p['rows']) == 6144 and len(q['rows']) == 7424
assert q['rows'][:6144] == p['rows']
assert [int(r['row_index']) for r in q['rows']] == list(range(7424))
for g in range(4):
    rows=[r for r in q['rows'] if int(r['gpu_id'])==g and int(r['row_index'])>=6144]
    assert len(rows)==320, (g,len(rows))
print('PREFIX_READY|parent=6144|target=7424|new_rows=1280|rows_per_gpu=320')
PY

echo "MANIFEST_READY|count=7424|file_sha256=$MAN_SHA|parent6144_sha256=$PARENT_SHA"
...
cat >"$RUNROOT/LAUNCH.json" <<EOF
{
  ...
  "b24_head": "$HEAD",
  "parent6144_runroot": "$PARENT6144",
  "parent6144_manifest_path": "$PARENT_MANIFEST",
  "parent6144_manifest_file_sha256": "$PARENT_SHA",
  "manifest_path": "$MANIFEST",
  "manifest_file_sha256": "$MAN_SHA",
  "target_count": 7424,
  "new_row_range": [6144, 7423],
  "new_rows": 1280,
  "rows_per_gpu": 320,
  "screen_purpose": "FIXED_CUMULATIVE_TOPUP_FOR_CLASS_B_TO_AT_LEAST_100",
  "gating_collection_classes": ["B"],
  "class_definitions_unchanged": true,
  "early_stop_within_tranche": false,
  "per_method_concurrency": 4,
  "baseline_min_free_mib": 10240,
  "hard_ceiling_mib": 52452,
  "gpu_fit_behavior": "WAIT_AND_RETRY_CALIBRATED_GATE",
  "parent_requirement": "ALL_6144_SHARDS_PASS_BEFORE_LAUNCH"
}
EOF
...
for GPU in 0 1 2 3; do
  nohup env \
    ...
    "$PY" scripts/b24/run_b24_2_7424_extension_shard.py \
      --manifest "$MANIFEST" \
      --parent-runroot "$PARENT6144" \
...
printf '%s\n' "$RUNROOT" >"$OUTROOT/B24_2_7424_LATEST_RUN.txt"

echo "B24_2_7424_LAUNCHED|runroot=$RUNROOT|head=$HEAD|manifest_sha256=$MAN_SHA|new_rows=1280|rows_per_gpu=320|gate_mib=$BASELINE_MIN_FREE_MIB"
echo "STATUS|bash $REPO/scripts/b24/status_b24_2_7424.sh $RUNROOT"
echo "RESUME_IF_NEEDED|bash $REPO/scripts/b24/resume_b24_2_7424.sh $RUNROOT"
