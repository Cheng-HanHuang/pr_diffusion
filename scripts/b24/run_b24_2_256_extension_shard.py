#!/usr/bin/env python3
"""Run one B24.2 cumulative-256 extension shard (rows 64--255 only).

The first 64 rows are an immutable parent checkpoint. This worker executes only
new rows assigned to one fixed physical GPU. It preserves the B24.1-validated
four-independent-process scheduling form for DAPS-4 and SITCOM-4.

Unlike the first 64-image worker, transient lack of free GPU memory causes a
recorded wait rather than an immediate shard failure. Completed rows remain
atomically resumable; incomplete row directories are preserved on --resume.
"""
...
def wait_for_fit(gpu: int, label: str) -> float:
    ...
    while True:
        ...
        if free >= MIN_FREE_BEFORE_LAUNCH_MIB:
            ...
            if waited > 0.5:
                print(f"GPU_FIT_READY|gpu={gpu}|label={label}|free_mib={free}|wait_s={waited:.1f}", flush=True)
            return waited
        elapsed = int(time.perf_counter() - start)
        if elapsed // 300 != last_print:
            last_print = elapsed // 300
            print(
                f"GPU_WAIT|gpu={gpu}|label={label}|free_mib={free}|required_mib={MIN_FREE_BEFORE_LAUNCH_MIB}|elapsed_s={elapsed}",
                flush=True,
            )
...
def run_group_after_fit(smoke, specs, gpu: int, memory_path: Path, label: str):
    while True:
        ...
        try:
            ...
            # Race: another job can enter between our fit observation and the
            # native helper's own pre-group fit gate. Retry only that gate.
...
def main() -> int:
    ...
    parent = read_json(args.parent_manifest)
    ...
    if not isinstance(parent_rows, list) or len(parent_rows) != PARENT_COUNT:
        raise RuntimeError(f"expected {PARENT_COUNT}-row parent manifest")
    if rows[:PARENT_COUNT] != parent_rows:
        raise RuntimeError("cumulative-256 manifest does not preserve exact 64-row prefix")

    selected = [r for r in shard_rows(rows)[args.shard] if int(r["row_index"]) >= PARENT_COUNT]
    if len(selected) != 48:
        raise RuntimeError(f"expected 48 extension rows in shard {args.shard}, got {len(selected)}")
    ...
    smoke = load_module(repo / "scripts/b24/run_b24_1_method_smoke.py", "b24_smoke_reuse_256")
    ...
    for row in selected:
        row_index = int(row["row_index"])
        image_id = str(row["image_id"]).zfill(5)
        image_dir = out / f"row{row_index:03d}_{image_id}"
        ...
        if args.resume and completion_path.is_file():
            ...
            print(
                f"IMAGE_REUSE|shard={args.shard}|row={row_index}|image={image_id}|class={value['class_label']}|"
                f"daps={float(value['daps_best_psnr_raw_rgb_db']):.4f}|sitcom={float(value['sitcom_best_psnr_raw_rgb_db']):.4f}",
                flush=True,
            )
            continue
        ...
        if image_dir.exists():
            ...
            print(f"PARTIAL_PRESERVED|shard={args.shard}|row={row_index}|image={image_id}|path={dest}", flush=True)

        image_dir.mkdir()
        image_start = time.perf_counter()
        print(f"IMAGE_START|shard={args.shard}|row={row_index}|image={image_id}", flush=True)

        # Fresh locked input for the manifest-frozen row.
        ...
        data_name = f"b24-256-{out.parent.name}-s{args.shard}-r{row_index:03d}-{image_id}".lower()
        ...
        try:
            ...
            daps = run_group_after_fit(smoke, daps_specs, args.gpu, image_dir / "daps_memory.tsv", f"row{row_index}:DAPS")
            total_wait += max(0.0, time.perf_counter() - before - float(daps["group_wall_seconds"]))
        finally:
            if config_path is not None and Path(config_path).exists(): Path(config_path).unlink()
            if data_dir is not None and Path(data_dir).exists(): shutil.rmtree(data_dir)
        ...
        for spec in sitcom_specs:
            ...
            mval["selection_rule"] = "B24.2 deterministic cumulative-256 screen row; frozen image/measurement/solver seeds"
            mval["selection_uses_method_outcome"] = False
            mval["b24_stage"] = "B24.2_256_EXTENSION"
            write_atomic(mpath, mval)
        before = time.perf_counter()
        sitcom = run_group_after_fit(smoke, sitcom_specs, args.gpu, image_dir / "sitcom_memory.tsv", f"row{row_index}:SITCOM")
        total_wait += max(0.0, time.perf_counter() - before - float(sitcom["group_wall_seconds"]))
        write_atomic(image_dir / "SITCOM_GROUP.json", sitcom)

        total_wait += wait_for_fit(args.gpu, f"row{row_index}:metrics")
        metric_env = os.environ.copy(); metric_env["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
        run_logged([
        ...
        completion = {
            ...
            "stage": "B24.2_256_EXTENSION",
            "manifest_file_sha256": manifest_sha,
            "parent_64_manifest_file_sha256": parent_sha,
        ...
        completed += 1; new += 1
        print(
            f"IMAGE_COMPLETE|shard={args.shard}|row={row_index}|image={image_id}|class={completion['class_label']}|"
            f"daps={completion['daps_best_psnr_raw_rgb_db']:.4f}|sitcom={completion['sitcom_best_psnr_raw_rgb_db']:.4f}|"
            f"wall_s={completion['image_wall_seconds']:.1f}", flush=True,
        )
