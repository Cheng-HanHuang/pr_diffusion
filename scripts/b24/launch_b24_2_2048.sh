...
[[ -f "$PARENT_POINTER" ]] || { echo "STOP|missing_parent256_pointer:$PARENT_POINTER"; exit 2; }
PARENT256=$(cat "$PARENT_POINTER")
PARENT_MANIFEST="$PARENT256/B24_2_baseline_256.json"
[[ -d "$PARENT256" ]] || { echo "STOP|missing_parent256:$PARENT256"; exit 2; }
[[ -f "$PARENT_MANIFEST" ]] || { echo "STOP|missing_parent256_manifest:$PARENT_MANIFEST"; exit 2; }
...
"$PY" -m py_compile \
  ...
  scripts/b24/run_b24_2_2048_extension_shard.py \
...
"$PY" - "$PARENT_MANIFEST" <<'PY'
import json,sys
from pathlib import Path
p=Path(sys.argv[1]); v=json.loads(p.read_text())
assert len(v['rows']) == 256
assert [int(r['row_index']) for r in v['rows']] == list(range(256))
print(f'PARENT256_MANIFEST_READY|path={p}|rows=256')
PY

STAMP=$(date -u +%Y%m%dT%H%M%SZ)
RUNROOT="$OUTROOT/B24_2_2048_extension_$STAMP"
mkdir -p "$RUNROOT/logs" "$RUNROOT/pids"
MANIFEST="$RUNROOT/B24_2_baseline_2048.json"
"$PY" scripts/b24/render_b24_baseline_manifest.py --count 2048 --out "$MANIFEST" \
...
assert len(p['rows']) == 256 and len(q['rows']) == 2048
assert q['rows'][:256] == p['rows']
assert [int(r['row_index']) for r in q['rows']] == list(range(2048))
for g in range(4):
    rows=[r for r in q['rows'] if int(r['gpu_id'])==g and int(r['row_index'])>=256]
    assert len(rows)==448, (g,len(rows))
print('PREFIX_READY|parent=256|target=2048|new_rows=1792|rows_per_gpu=448')
PY

echo "MANIFEST_READY|count=2048|file_sha256=$MAN_SHA|parent256_sha256=$PARENT_SHA"
...
cat >"$RUNROOT/LAUNCH.json" <<EOF
{
  ...
  "b24_head": "$HEAD",
  "parent256_runroot": "$PARENT256",
  "parent256_manifest_path": "$PARENT_MANIFEST",
  "parent256_manifest_file_sha256": "$PARENT_SHA",
  "manifest_path": "$MANIFEST",
  "manifest_file_sha256": "$MAN_SHA",
  "target_count": 2048,
  "new_row_range": [256, 2047],
  "new_rows": 1792,
  "rows_per_gpu": 448,
  ...
  "parent_handoff": "EACH_GPU_WAITS_FOR_ITS_OWN_PARENT256_SHARD_PASS"
}
EOF
...
for GPU in 0 1 2 3; do
  nohup env \
    ...
    "$PY" scripts/b24/run_b24_2_2048_extension_shard.py \
      --manifest "$MANIFEST" \
      --parent-runroot "$PARENT256" \
...
printf '%s\n' "$RUNROOT" >"$OUTROOT/B24_2_2048_LATEST_RUN.txt"

echo "B24_2_2048_LAUNCHED|runroot=$RUNROOT|head=$HEAD|manifest_sha256=$MAN_SHA|new_rows=1792|rows_per_gpu=448|gate_mib=$BASELINE_MIN_FREE_MIB"
echo "PARENT_HANDOFF|gpu2_may_wait_for_existing_256_shard_without_manual_relaunch"
echo "STATUS|bash $REPO/scripts/b24/status_b24_2_2048.sh $RUNROOT"
