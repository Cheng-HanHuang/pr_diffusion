#!/usr/bin/env python3
"""Run one B24.2 cumulative-6144 extension shard (rows 2048--6143 only).

The completed cumulative-2048 campaign is the immutable parent checkpoint. This
...
atomically resumable. On resume, only exact B24-6144-owned DAPS scratch for an
...
PARENT_ROWS_PER_SHARD = PARENT_COUNT // 4 - 64  # 448 extension rows per 2048 parent shard
POLL_SECONDS = 60
...
def wait_for_parent_shard(parent_runroot: Path, shard: int) -> dict:
    ...
    if not summary_path.is_file():
        raise RuntimeError(f"missing completed 2048 parent shard summary: {summary_path}")
    value = read_json(summary_path)
    if (
        value.get("status") != "PASS"
        or int(value.get("completed", -1)) != EXPECTED_ROWS_PER_SHARD - 576
        or value.get("stage") != "B24.2_2048_EXTENSION"
    ):
        # EXPECTED_ROWS_PER_SHARD=1024; 1024-576=448. Keep the expected parent
        # count visibly tied to the established completed 2048 campaign.
        raise RuntimeError(f"bad parent-2048 shard summary: {summary_path}: {value}")
    print(
        f"PARENT2048_READY|shard={shard}|completed={value['completed']}|summary={summary_path}",
        flush=True,
    )
    return value
...
def run_group_after_fit(smoke, specs, gpu: int, memory_path: Path, label: str):
    # Override the generic B24.1 52-GiB default only for this calibrated stage.
...
def cleanup_daps_scratch(data_name: str) -> None:
    """Remove only the exact B24-6144 scratch pair for an incomplete row."""
    if not SAFE_NAME.fullmatch(data_name) or not data_name.startswith("b24-6144-"):
        raise RuntimeError(f"refusing unsafe/non-B24-6144 scratch cleanup: {data_name!r}")
...
def main() -> int:
    ...
    parent_manifest_path = parent_runroot / "B24_2_baseline_2048.json"
    ...
    if rows[:PARENT_COUNT] != parent_rows:
        raise RuntimeError("cumulative-6144 manifest does not preserve exact 2048-row prefix")
    ...
    smoke = load_module(
        repo / "scripts/b24/run_b24_1_method_smoke.py", "b24_smoke_reuse_6144"
    )
    smoke.MIN_FREE_MIB = CALIBRATED_MIN_FREE_MIB
    ...
    for row in selected:
        ...
        data_name = (
            f"b24-6144-{out.parent.name}-s{args.shard}-r{row_index:04d}-{image_id}"
        ).lower()
        ...
        for spec in sitcom_specs:
            ...
            mval["selection_rule"] = (
                "B24.2 deterministic cumulative-6144 screen row; "
                "frozen image/measurement/solver seeds"
            )
            mval["selection_uses_method_outcome"] = False
            mval["b24_stage"] = "B24.2_6144_EXTENSION"
        ...
        completion = {
            ...
            "stage": "B24.2_6144_EXTENSION",
            "manifest_file_sha256": manifest_sha,
            "parent_2048_manifest_file_sha256": parent_sha,
