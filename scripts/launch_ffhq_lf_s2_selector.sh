...
# Launch the lightweight LF/S2 selector experiment, then apply the seed tie-break
# postprocessor when the run finishes.
#
# Usage from repo root:
#   bash scripts/launch_ffhq_lf_s2_selector.sh
#
# Optional overrides:
#   GPU=0 SEEDS=100,101 MAX_IMAGES=25 SKIP_LPIPS=1 SEED_TIE_THRESHOLD=0.00005 bash scripts/launch_ffhq_lf_s2_selector.sh
...
OUTROOT=${OUTROOT:-$ROOT/np_ffhq_lf_s2_selector_tiebreak}
GPU=${GPU:-0}
SEEDS=${SEEDS:-100,101}
...
SEED_TIE_THRESHOLD=${SEED_TIE_THRESHOLD:-0.00005}
...
LOG="$OUTROOT/logs/lf_s2_selector_tiebreak_$(date +%Y%m%d_%H%M%S).log"
echo "[launch] GPU=$GPU log=$LOG"
(
  set -euo pipefail
  CUDA_VISIBLE_DEVICES="$GPU" python scripts/pr_external_difffpr_np_guided_lf_s2_selector.py \
    --data_root "$DATA_ROOT" \
  ...
  echo "[postprocess] applying seed tie-break threshold=$SEED_TIE_THRESHOLD"
  python scripts/apply_lf_s2_seed_tiebreak.py \
    --run_dir "$OUTROOT" \
    --seed_tie_threshold "$SEED_TIE_THRESHOLD" \
    --out_prefix selected_tiebreak_thr${SEED_TIE_THRESHOLD}
) > "$LOG" 2>&1 &
...
Expected files inside the latest timestamped lf_s2_selector_* directory:
  configs.csv
  run_level.csv
  selected_image_level.csv
  selected_summary.csv
  selected_tiebreak_thr${SEED_TIE_THRESHOLD}_image_level.csv
  selected_tiebreak_thr${SEED_TIE_THRESHOLD}_summary.csv
EOF
