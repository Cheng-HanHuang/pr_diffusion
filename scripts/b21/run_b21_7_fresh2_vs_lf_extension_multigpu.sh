...
GPUS=${GPUS:-"0 1 2 3"}
EXTRA_SEED0=${EXTRA_SEED0:-13000}
ANN_STEPS=${ANN_STEPS:-400}
DIFF_STEPS=${DIFF_STEPS:-5}
THETA=${THETA:-0.7}
B21_FORCE=${B21_FORCE:-0}
B21_SMOKE_ONLY=${B21_SMOKE_ONLY:-0}
EXT_ROOT=${EXT_ROOT:-$FRESH_OUT/fresh2_vs_lf_extension}
LOGDIR="$EXT_ROOT/logs"
MANIFEST="$EXT_ROOT/manifest.tsv"
ACTIVE_MANIFEST="$EXT_ROOT/active_manifest.tsv"
DONE="$EXT_ROOT/done.tsv"
FAIL="$EXT_ROOT/fail.tsv"
METRIC_ANALYZER="$REPO/scripts/b19/analyze_daps_exact_final_loss_selector.py"
FINAL_ANALYZER="$REPO/scripts/b21/analyze_b21_7_fresh2_vs_lf_allocation.py"
ANALYSIS="$EXT_ROOT/allocation_analysis_theta${THETA}"
REPORT="$REPO/docs/b21/b21_7_fresh2_vs_lf_allocation.md"

cd "$REPO"
mkdir -p "$EXT_ROOT" "$LOGDIR"
...
# Recreate exact data configs from the already frozen source manifest.
...
: > "$DONE"
: > "$FAIL"
printf "job_id\timage_id\tcase_id\tgpu\tbase_seed\textra_seed\tmeasurement_path\n" > "$MANIFEST"
...
while IFS=$'\t' read -r job_id image case_id old_gpu base_seed hio_seed warm_seed inject_step measurement_path; do
  ...
  extra_seed=$((EXTRA_SEED0 + job_id))
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    "$job_id" "$image" "$case_id" "$gpu" "$base_seed" "$extra_seed" "$measurement_path" >> "$MANIFEST"
done < "$SOURCE_MANIFEST"

rows=$(($(wc -l < "$MANIFEST") - 1))
[[ "$rows" -eq 80 ]] || { echo "[fatal] expected 80 extension rows, found $rows" >&2; exit 2; }
...

{
  ...
  echo "fresh_rows=$FRESH_ROWS"
  echo "source_manifest=$SOURCE_MANIFEST"
  echo "gpus=$GPUS"
  echo "extra_seed0=$EXTRA_SEED0"
...
} > "$EXT_ROOT/launch_env.txt"

run_extra() {
  local job_id="$1" image="$2" case_id="$3" gpu="$4" extra_seed="$5" meas="$6"
  ...
  start=$(date +%s)
  echo "[run][gpu=$gpu] job=$job_id image=$image case=$case_id seed=$extra_seed candidate=$candidate"
  (
    cd "$DAPS"
    env \
      -u B21_CONT_ENABLE -u B21_CONT_STATE_PATH -u B21_CONT_NOISE_SEED \
      -u B21_SAVE_STATE_STEPS \
      B21_START_NOISE_SEED="$extra_seed" \
      B21_SOURCE_SEED="$extra_seed" \
      ...
      "$PYTHON_BIN" posterior_sample.py \
        ...
        seed="$extra_seed" num_runs=1 name="$candidate" save_dir="$save_dir" \
  ...
  awk -F'\t' '$1 != "base_extra"' "$timing_file" > "${timing_file}.tmp" || true
...
for gpu in "${GPU_ARR[@]}"; do
  (
    ...
    while IFS=$'\t' read -r job_id image case_id assigned_gpu base_seed extra_seed meas; do
      [[ -z "${job_id:-}" ]] && continue
      echo "[case start] job=$job_id image=$image case=$case_id gpu=$assigned_gpu"
      if run_extra "$job_id" "$image" "$case_id" "$assigned_gpu" "$extra_seed" "$meas"; then
...
if [[ "$B21_SMOKE_ONLY" == "1" ]]; then
  echo "[smoke-only] completed first independent base-extra case; analyzer intentionally skipped"
  exit 0
fi
...
"$PYTHON_BIN" "$FINAL_ANALYZER" \
  ...
  --report "$REPORT" \
  | tee "$EXT_ROOT/analyzer_stdout.txt"

echo "[done] B21.7 Fresh2 versus LF allocation artifacts: $EXT_ROOT"
